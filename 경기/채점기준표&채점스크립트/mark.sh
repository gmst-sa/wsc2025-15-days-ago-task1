...
aws ec2 describe-images --image-ids $(aws ec2 describe-instances --filters Name=tag:Name,Values=ws25-ec2-bastion --query "Reservations[].Instances[].ImageId" --output text) --query "Images[].Name" --output text; aws ec2 describe-instances --filters Name=tag:Name,Values=ws25-ec2-bastion --query "Reservations[].Instances[].InstanceType" --output text; aws ec2 describe-subnets --subnet-ids $(aws ec2 describe-instances --filters Name=tag:Name,Values=ws25-ec2-bastion --query "Reservations[].Instances[].SubnetId" --output text) --query "Subnets[].Tags[?Key=='Name'].Value | [0]" --output text; aws ec2 describe-addresses --filters Name=instance-id,Values=$(aws ec2 describe-instances --filters Name=tag:Name,Values=ws25-ec2-bastion --query "Reservations[].Instances[].InstanceId" --output text) --query "Addresses[].PublicIp"
...
aws ecs describe-services --cluster ws25-ecs-cluster --services ws25-ecs-green --region ap-northeast-2 --query "services[0].taskDefinition" --output text | xargs -I{} aws ecs describe-task-definition --task-definition {} --region ap-northeast-2 --output json | jq -r '((.taskDefinition.containerDefinitions | all((.environment? // []) | length == 0))) as $ok | if $ok then "True" else "False" end'
...
aws ecs describe-services --cluster ws25-ecs-cluster --services ws25-ecs-red --region ap-northeast-2 --query "services[0].taskDefinition" --output text | xargs -I{} aws ecs describe-task-definition --task-definition {} --region ap-northeast-2 --output json | jq -r '((.taskDefinition.containerDefinitions | all((.environment? // []) | length == 0))) as $ok | if $ok then "True" else "False" end'
echo ""
