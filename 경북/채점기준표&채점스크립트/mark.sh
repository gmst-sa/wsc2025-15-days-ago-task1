...
echo =====5-2=====
kubectl get deployment green-deploy -n skills -o jsonpath='{range .spec.template.spec.containers[*].env[?(@.valueFrom.secretKeyRef)]}{.name}{"\n"}{end}'
kubectl get deployment green-deploy -n skills -o jsonpath='{range .spec.template.spec.containers[*].envFrom[?(@.secretRef)]}{.secretRef.name}{"\n"}{end}'
echo
